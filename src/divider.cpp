    ...
    void agents_callback(std::shared_ptr<farmbot_interfaces::msg::Agents> msg) {
        ...
        for (auto agent : msg->beacons) {
            ...
            rclcpp::Publisher<farmbot_interfaces::msg::Lines>::SharedPtr border_pub =
                node_->create_publisher<farmbot_interfaces::msg::Lines>(agent_name + "/pln/border", 10);
            rclcpp::Publisher<farmbot_interfaces::msg::Lines>::SharedPtr swaths_pub =
                node_->create_publisher<farmbot_interfaces::msg::Lines>(agent_name + "/pln/swaths", 10);
            rclcpp::Publisher<farmbot_interfaces::msg::Lines>::SharedPtr headland_pub =
                node_->create_publisher<farmbot_interfaces::msg::Lines>(agent_name + "/pln/headland", 10);
            all_border_pubs_.push_back(std::make_pair(agent_name, border_pub));
            all_swaths_pubs_.push_back(std::make_pair(agent_name, swaths_pub));
            all_headland_pubs_.push_back(std::make_pair(agent_name, headland_pub));
        }
