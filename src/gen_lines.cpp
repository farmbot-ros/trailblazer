...
class FieldProcessorNode : public rclcpp::Node {
  ...
  private:
    ...
    void gen_swaths() {
        std::vector<std::array<double, 6>> points = get_field();
        if (points.empty()) {
    ...
    std::vector<std::array<double, 6>> get_field(std::string geojson_file_path = "") {
        ...
        for (int i = 0; i < result.get()->points; i++) {
            points.emplace_back(lpts[i].x, lpts[i].y, lpts[i].z, gpts[i].latitude, gpts[i].longitude, gpts[i].altitude);
        }
        return points;
    }
